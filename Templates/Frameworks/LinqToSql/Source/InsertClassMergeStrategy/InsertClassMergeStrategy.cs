...
namespace CodeSmith.Engine
{
    public class InsertClassMergeStrategy : IMergeStrategy
    {
        ...
        public InsertClassMergeStrategy(string language, string className, bool preserveClassAttributes)
        {
        ...
        public void Init(string init)
        {
            ...
            if (attribs.ContainsKey("PreserveClassAttributes"))
                this.PreserveClassAttributes = Boolean.TryParse((string)attribs["PreserveClassAttributes"], out _preserveClassAttributes);
        }
        public string Merge(CodeTemplate context, string sourceContent, string templateOutput)
        {
            ...
            // Parse Source
            CodeFileParser sourceParser = CodeFileParser.Create(sourceContent, this.supportedLanguage);
            ...
            // Parse Template
            CodeFileParser templateParser = CodeFileParser.Create(templateOutput, this.supportedLanguage);
        ...
        {
            return (!PreserveClassAttributes && typeDeclaration.Attributes.Count > 0)
