                ...
                foreach (AssociationMember member in sc.AssociationMembers)
                {
                    ...
                    if(isObjectFactory)
                    {
                        if (sc.IsChild)
                        {
                            ...
                            propertyName = isObjectFactory ? 
                                string.Format("item.{0}{1}", Util.NamingConventions.PropertyName(member.AssociatedColumn.ColumnName), nullable) :
                                string.Format("{0}{1}", Util.NamingConventions.VariableName(member.AssociatedColumn.ColumnName), nullable);
                        }
                        else
                        {
                            ...
                            propertyName = isObjectFactory ? 
                                string.Format("item.{0}{1}", Util.NamingConventions.PropertyName(member.ColumnName), nullable) : 
                                string.Format("{0}{1}", Util.NamingConventions.VariableName(member.ColumnName), nullable);
                        }
                    }
