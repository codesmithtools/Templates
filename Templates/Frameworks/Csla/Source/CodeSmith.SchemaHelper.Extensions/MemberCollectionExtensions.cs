...
namespace CodeSmith.SchemaHelper
{
    ...
    public static class MemberCollectionExtensions
    {
        ...
        //    }
            ...
            //                            ? associationMember.AssociatedColumn.SystemType
            //                            : associationMember.AssociatedColumn.SystemType.TrimEnd(new[] { '?' });
        ...
        public static string BuildDataBaseColumns(this List<Member> members)
        {
        //    return BuildDataBaseColumns(members, new List<AssociationMember>());
        //}

        //public static string BuildDataBaseColumns(this List<Member> members, List<AssociationMember> associationMembers)
        //{
            string columnNames = string.Empty;
        ...
        //            if(usePropertyName)
        //                propertyName = string.Format("{0}.{1}", associationMember.PropertyName, NamingConventions.PropertyName(associationMember.AssociatedColumn.Name));
        //            else
        //                propertyName = string.Format("{0}.{1}", associationMember.VariableName, NamingConventions.PropertyName(associationMember.AssociatedColumn.Name));
        //        }
        ...
        //            string propertyName = string.Format("{0}.{1}", associationMember.VariableName, NamingConventions.PropertyName(associationMember.AssociatedColumn.Name));
        //            if (usePropertyName)
        //                propertyName = string.Format("{0}.{1}", associationMember.PropertyName, NamingConventions.PropertyName(associationMember.AssociatedColumn.Name));
        ...
        //{
        //    return members.BuildCommandParameters(false);
        //}

        //public static string BuildCommandParameters(this List<Member> members, bool isObjectFactory)
        //{
        //    return members.BuildCommandParameters(isObjectFactory, false);
        //}
        ...
        //{
        //    return BuildCommandParameters(members,, false, false);
        //}
        ...
        //    }
            ...
            //    {
            //        if (Configuration.Instance.TargetLanguage == LanguageEnum.VB)
            //            cast = associationMember.IsNullable ? string.Format("IIf({0}{1}.HasValue, DirectCast({0}{1}.Value.Date, DateTime), System.DBNull.Value))", castPrefix, propertyName)
            //                                                : string.Format("DirectCast({0}{1}.Date, DateTime))", castPrefix, propertyName);
            //        else
            //            cast = associationMember.IsNullable ? string.Format("(DateTime?){0}{1});", castPrefix, propertyName)
            //                                                : string.Format("(DateTime){0}{1});", castPrefix, propertyName);
            //    }
            //    else
            //        cast = Configuration.Instance.TargetLanguage == LanguageEnum.VB ? string.Format("{0}{1})", castPrefix, propertyName) : string.Format("{0}{1});", castPrefix, propertyName);
        ...
        //{
            ...
            foreach (Member member in members)
            {
                ...
                //{
                //    if (association.AssociatedColumn.ColumnName == member.ColumnName)
                //    {
