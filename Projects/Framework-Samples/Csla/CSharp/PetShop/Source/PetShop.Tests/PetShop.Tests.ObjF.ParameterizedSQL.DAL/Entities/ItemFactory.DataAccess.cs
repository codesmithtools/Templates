    ...
    public partial class ItemFactory : ObjectFactory
    {
        ...
        private void DoUpdate(ref Item item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
                ...
                const string commandText = "UPDATE [dbo].[Item]  SET [ItemId] = @p_ItemId, [ProductId] = @p_ProductId, [ListPrice] = @p_ListPrice, [UnitCost] = @p_UnitCost, [Supplier] = @p_Supplier, [Status] = @p_Status, [Name] = @p_Name, [Image] = @p_Image WHERE [ItemId] = @p_ItemId AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[Item] WHERE ItemId = @p_ItemId";
                using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(SqlCommand command = new SqlCommand(commandText, connection))
