...
namespace PetShop.Tests.ObjF.ParameterizedSQL.DAL
{
    public partial class LineItemFactory : ObjectFactory
    {
        ...
        private void DoInsert(ref LineItem item, bool stopProccessingChildren)
        {
            ...
            const string commandText = "INSERT INTO [dbo].[LineItem] ([OrderId], [LineNum], [ItemId], [Quantity], [UnitPrice]) VALUES (@p_OrderId, @p_LineNum, @p_ItemId, @p_Quantity, @p_UnitPrice); SELECT [timestamp] FROM [dbo].[LineItem] WHERE OrderId = @p_OrderId";
            using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
            {
                connection.Open();
                using(SqlCommand command = new SqlCommand(commandText, connection))
        ...
        private void DoUpdate(ref LineItem item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
                if(item.OriginalOrderId != item.OrderId || item.OriginalLineNum != item.LineNum)
                {
                    ...
                    // Delete the old.
                    Delete(new LineItemCriteria {OrderId = item.OriginalOrderId, LineNum = item.OriginalLineNum});
                ...
                const string commandText = "UPDATE [dbo].[LineItem]  SET [OrderId] = @p_OrderId, [LineNum] = @p_LineNum, [ItemId] = @p_ItemId, [Quantity] = @p_Quantity, [UnitPrice] = @p_UnitPrice WHERE [OrderId] = @p_OrderId AND [LineNum] = @p_LineNum AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[LineItem] WHERE OrderId = @p_OrderId";
                using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(SqlCommand command = new SqlCommand(commandText, connection))
