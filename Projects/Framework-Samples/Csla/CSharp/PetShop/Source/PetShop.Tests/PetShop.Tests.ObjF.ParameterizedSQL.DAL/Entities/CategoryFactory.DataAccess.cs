...
namespace PetShop.Tests.ObjF.ParameterizedSQL.DAL
{
    public partial class CategoryFactory : ObjectFactory
    {
        ...
        public Category Fetch(CategoryCriteria criteria)
        {
            ...
            Category item;
            string commandText = string.Format("SELECT [CategoryId], [Name], [Descn], [timestamp] FROM [dbo].[Category] {0}", ADOHelper.BuildWhereStatement(criteria.StateBag));
        ...
        private void DoInsert(ref Category item, bool stopProccessingChildren)
        {
            ...
            const string commandText = "INSERT INTO [dbo].[Category] ([CategoryId], [Name], [Descn]) VALUES (@p_CategoryId, @p_Name, @p_Descn); SELECT [timestamp] FROM [dbo].[Category] WHERE CategoryId = @p_CategoryId";
            using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
            {
                connection.Open();
                using(SqlCommand command = new SqlCommand(commandText, connection))
        ...
        private void DoUpdate(ref Category item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
                ...
                const string commandText = "UPDATE [dbo].[Category]  SET [CategoryId] = @p_CategoryId, [Name] = @p_Name, [Descn] = @p_Descn WHERE [CategoryId] = @p_CategoryId AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[Category] WHERE CategoryId = @p_CategoryId";
                using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(SqlCommand command = new SqlCommand(commandText, connection))
