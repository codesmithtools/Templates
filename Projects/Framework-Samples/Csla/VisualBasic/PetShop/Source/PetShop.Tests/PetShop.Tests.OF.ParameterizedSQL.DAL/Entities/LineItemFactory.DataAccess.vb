...
Public Partial Class LineItemFactory
    ...
    Public Function Fetch(ByVal criteria As LineItemCriteria) As LineItem
        ...
        Dim commandText As String = String.Format("SELECT [OrderId], [LineNum], [ItemId], [Quantity], [UnitPrice], [timestamp] FROM [dbo].[LineItem] {0}", ADOHelper.BuildWhereStatement(criteria.StateBag))
    ...
    Private Sub DoInsert(ByRef item As LineItem, ByVal stopProccessingChildren As Boolean)
        ...
        Const commandText As String = "INSERT INTO [dbo].[LineItem] ([OrderId], [LineNum], [ItemId], [Quantity], [UnitPrice]) VALUES (@p_OrderId, @p_LineNum, @p_ItemId, @p_Quantity, @p_UnitPrice); SELECT [timestamp] FROM [dbo].[LineItem] WHERE OrderId = @p_OrderId"
        Using connection As New SqlConnection(ADOHelper.ConnectionString)
            connection.Open()
            Using command As New SqlCommand(commandText, connection)
    ...
    Private Sub DoUpdate(ByRef item As LineItem, ByVal stopProccessingChildren As Boolean)
        ...
        If (item.IsDirty) Then
            ...
            Const commandText As String = "UPDATE [dbo].[LineItem]  SET [OrderId] = @p_OrderId, [LineNum] = @p_LineNum, [ItemId] = @p_ItemId, [Quantity] = @p_Quantity, [UnitPrice] = @p_UnitPrice WHERE [OrderId] = @p_OrderId AND [LineNum] = @p_LineNum AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[LineItem] WHERE OrderId = @p_OrderId"
            Using connection As New SqlConnection(ADOHelper.ConnectionString)
                connection.Open()
                Using command As New SqlCommand(commandText, connection)
