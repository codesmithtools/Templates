...
namespace QuickStart
{
    public class WebsiteProjectCreator : VsProjectCreator
    {
        ...
        public override SolutionItem CreateProject(string projectName, params SolutionItem[] dependancies)
        {
            ...
            string webprojPath = QuickStartHelper.FindFileInDirectory(webprojFile, projectPath.DirectoryPath);
        ...
        {
            string globalPath = QuickStartHelper.FindFileInDirectory("Global.asax", projectPath.DirectoryPath);
            ...
            if (ProjectBuilder.Language == LanguageEnum.CSharp)
            {
                ...
                @".",
                ProjectBuilder.SourceDatabase.Name,
            ...
            QuickStartHelper.FindAndReplace(globalPath, contextLine, contextReplace);
        }
        private void AddWebServicesToProj(PathHelper projectPath)
        {
            if (ProjectBuilder.IncludeDataServices)
            {
                ...
                string dataContextName = (ProjectBuilder.Language == LanguageEnum.CSharp)
                    ...
                    : @"\[\[class name\]\]";
                QuickStartHelper.FindAndReplace(fileName, dataContextName,
                ...
                QuickStartHelper.FindAndReplace(fileName, @"</configuration>", sb.ToString());
                ...
                QuickStartHelper.FindAndReplace(fileName, @"      </assemblies>", sb.ToString());
            }
        }

        private void UpdateWebServicesVariables(string fileName)
        {
            QuickStartHelper.FindAndReplace(fileName, @"\$rootnamespace\$", ProjectBuilder.InterfaceProjectName);
            QuickStartHelper.FindAndReplace(fileName, @"\$safeitemname\$", DataServiceName);
        }
