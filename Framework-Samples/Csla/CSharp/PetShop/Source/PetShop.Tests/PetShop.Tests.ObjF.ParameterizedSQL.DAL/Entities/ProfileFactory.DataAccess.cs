...
namespace PetShop.Tests.ObjF.ParameterizedSQL.DAL
{
    public partial class ProfileFactory : ObjectFactory
    {
        ...
        private void DoUpdate(ref Profile item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
                const string commandText = "UPDATE [dbo].[Profiles] SET [Username] = @p_Username, [ApplicationName] = @p_ApplicationName, [IsAnonymous] = @p_IsAnonymous, [LastActivityDate] = @p_LastActivityDate, [LastUpdatedDate] = @p_LastUpdatedDate WHERE [UniqueID] = @p_UniqueID; SELECT  FROM [dbo].[Profiles] WHERE [UniqueID] = @p_UniqueID";
                using (var connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(var command = new SqlCommand(commandText, connection))
