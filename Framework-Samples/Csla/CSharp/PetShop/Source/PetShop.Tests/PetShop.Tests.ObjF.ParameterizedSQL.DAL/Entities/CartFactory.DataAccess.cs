...
namespace PetShop.Tests.ObjF.ParameterizedSQL.DAL
{
    public partial class CartFactory : ObjectFactory
    {
        ...
        private void DoUpdate(ref Cart item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
                const string commandText = "UPDATE [dbo].[Cart] SET [UniqueID] = @p_UniqueID, [ItemId] = @p_ItemId, [Name] = @p_Name, [Type] = @p_Type, [Price] = @p_Price, [CategoryId] = @p_CategoryId, [ProductId] = @p_ProductId, [IsShoppingCart] = @p_IsShoppingCart, [Quantity] = @p_Quantity WHERE [CartId] = @p_CartId; SELECT  FROM [dbo].[Cart] WHERE [CartId] = @p_CartId";
                using (var connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(var command = new SqlCommand(commandText, connection))
