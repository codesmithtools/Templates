...
namespace PetShop.Business
{
    ...
    public partial class Account : BusinessBase<Account>
    {
        ...
        // ManyToOne
        private static readonly PropertyInfo<Profile> _profilesProperty = RegisterProperty<Profile>(p => p.Profiles, Csla.RelationshipTypes.Child);
        public Profile Profiles
        {
            get
            {
                bool cancel = false;
                OnChildLoading(_profilesProperty, ref cancel);

                if (!cancel)
                {
                    if(!FieldManager.FieldExists(_profilesProperty))
                    {
                        var criteria = new PetShop.Business.ProfileCriteria {UniqueID = UniqueID};
                        
#if SILVERLIGHT
                        //MarkBusy();
                        PetShop.Business.Profile.ExistsAsync(criteria, (o,e) =>
                        {
                            if(e.Error != null)
                                throw e.Error;
                                ...
                                PetShop.Business.Profile.GetByUniqueIDAsync(UniqueID, (o1, e1) =>
                                    {
                                        ...
                                        this.LoadProperty(_profilesProperty, e1.Object);
