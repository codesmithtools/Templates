...
namespace PetShop.Business
{
    ...
    public partial class Category : BusinessBase<Category>
    {
            ...
            get
            {
                ...
                if (!cancel)
                {
                    if(!FieldManager.FieldExists(_productsProperty))
                    {
                        ...
                        if((true))
                        {
                            PetShop.Business.ProductList.ExistsAsync(criteria).ContinueWith(t =>
                            {
                                if (!t.Result)
                                {
                                    PetShop.Business.ProductList.NewListAsync().ContinueWith(i =>
                                    {
                                        this.LoadProperty(_productsProperty, i.Result);
                                ...
                                {
                                    PetShop.Business.ProductList.GetByCategoryIdAsync(CategoryId).ContinueWith(i =>
                                    {
                                        this.LoadProperty(_productsProperty, i.Result);
        ...
        internal static async Task<Category> NewCategoryChildAsync()
        {
            return await DataPortal.CreateAsync<Category>();
        }

        internal static async Task<Category> GetByCategoryIdChildAsync(System.String categoryId)
        {
            var criteria = new CategoryCriteria{ CategoryId = categoryId};
