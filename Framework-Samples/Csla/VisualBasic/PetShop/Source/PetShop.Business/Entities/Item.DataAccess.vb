...
#Region "Root Data Access"
        ...
        Protected Overrides Sub DataPortal_Update()
            ...
            Const commandText As String = "UPDATE [dbo].[Item] SET [ItemId] = @p_ItemId, [ProductId] = @p_ProductId, [ListPrice] = @p_ListPrice, [UnitCost] = @p_UnitCost, [Supplier] = @p_Supplier, [Status] = @p_Status, [Name] = @p_Name, [Image] = @p_Image WHERE [ItemId] = @p_OriginalItemId; SELECT [ItemId] FROM [dbo].[Item] WHERE [ItemId] = @p_OriginalItemId"
            Using connection As New SqlConnection(ADOHelper.ConnectionString)
                connection.Open()
                Using command As New SqlCommand(commandText, connection)
...
#Region "Child_Insert"
        ...
        Private Sub Child_Insert(ByVal product As Product, ByVal supplier As Supplier, ByVal connection As SqlConnection)
            Dim cancel As Boolean = False
            OnChildInserting(product, supplier, connection, cancel)
            If (cancel) Then
                Return
            End If
...
#Region "Child_Update"
        ...
        Private Sub Child_Update(ByVal product As Product, ByVal supplier As Supplier, ByVal connection As SqlConnection)
            Dim cancel As Boolean = False
            OnChildUpdating(product, supplier, connection, cancel)
            If (cancel) Then
                Return
            End If
