            ...
            get
            {
                if(!FieldManager.FieldExists(_productsProperty))
                {
                    ...
                    if(IsNew || !PetShop.Business.ProductList.Exists(criteria))
                        PetShop.Business.ProductList.NewListAsync((o, e) =>
                            {
                                if (e.Error != null)
                                    throw e.Error; 

                                this.LoadProperty(_productsProperty, e.Object);

                                MarkIdle();
                                OnPropertyChanged(_productsProperty);
                            });
                    else
                        PetShop.Business.ProductList.GetByCategoryIdAsync(CategoryId, (o, e) =>
                            {
                                if (e.Error != null)
                                    throw e.Error; 
