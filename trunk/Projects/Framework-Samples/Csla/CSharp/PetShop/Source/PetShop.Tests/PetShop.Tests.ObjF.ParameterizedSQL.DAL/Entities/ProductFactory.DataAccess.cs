...
namespace PetShop.Tests.ObjF.ParameterizedSQL.DAL
{
    public partial class ProductFactory : ObjectFactory
    {
        ...
        public Product Fetch(ProductCriteria criteria)
        {
            ...
            Product item;
            string commandText = string.Format("SELECT [ProductId], [CategoryId], [Name], [Descn], [Image], [timestamp] FROM [dbo].[Product] {0}", ADOHelper.BuildWhereStatement(criteria.StateBag));
        ...
        private void DoInsert(ref Product item, bool stopProccessingChildren)
        {
            ...
            const string commandText = "INSERT INTO [dbo].[Product] ([ProductId], [CategoryId], [Name], [Descn], [Image]) VALUES (@p_ProductId, @p_CategoryId, @p_Name, @p_Descn, @p_Image); SELECT [timestamp] FROM [dbo].[Product] WHERE ProductId = @p_ProductId";
            using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
            {
                connection.Open();
                using(SqlCommand command = new SqlCommand(commandText, connection))
        ...
        private void DoUpdate(ref Product item, bool stopProccessingChildren)
        {
            ...
            if (item.IsDirty)
            {
				...
				itemToUpdate.ProductId = item.ProductId;
                    ...
                    // Delete the old.
                    Delete(new ProductCriteria {ProductId = item.OriginalProductId});
                ...
                const string commandText = "UPDATE [dbo].[Product]  SET [ProductId] = @p_ProductId, [CategoryId] = @p_CategoryId, [Name] = @p_Name, [Descn] = @p_Descn, [Image] = @p_Image WHERE [ProductId] = @p_ProductId AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[Product] WHERE ProductId = @p_ProductId";
                using (SqlConnection connection = new SqlConnection(ADOHelper.ConnectionString))
                {
                    connection.Open();
                    using(SqlCommand command = new SqlCommand(commandText, connection))
