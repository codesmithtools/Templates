...
Public Partial Class ProductFactory
    ...
    Public Function Fetch(ByVal criteria As ProductCriteria) As Product
        ...
        Dim commandText As String = String.Format("SELECT [ProductId], [CategoryId], [Name], [Descn], [Image], [timestamp] FROM [dbo].[Product] {0}", ADOHelper.BuildWhereStatement(criteria.StateBag))
    ...
    Private Sub DoInsert(ByRef item As Product, ByVal stopProccessingChildren As Boolean)
        ...
        Const commandText As String = "INSERT INTO [dbo].[Product] ([ProductId], [CategoryId], [Name], [Descn], [Image]) VALUES (@p_ProductId, @p_CategoryId, @p_Name, @p_Descn, @p_Image); SELECT [timestamp] FROM [dbo].[Product] WHERE ProductId = @p_ProductId"
        Using connection As New SqlConnection(ADOHelper.ConnectionString)
            connection.Open()
            Using command As New SqlCommand(commandText, connection)
    ...
    Private Sub DoUpdate(ByRef item As Product, ByVal stopProccessingChildren As Boolean)
		...
		itemToUpdate.ProductId = item.ProductId
            ...
            Const commandText As String = "UPDATE [dbo].[Product]  SET [ProductId] = @p_ProductId, [CategoryId] = @p_CategoryId, [Name] = @p_Name, [Descn] = @p_Descn, [Image] = @p_Image WHERE [ProductId] = @p_ProductId AND [timestamp] = @p_timestamp; SELECT [timestamp] FROM [dbo].[Product] WHERE ProductId = @p_ProductId"
            Using connection As New SqlConnection(ADOHelper.ConnectionString)
                connection.Open()
                Using command As New SqlCommand(commandText, connection)
